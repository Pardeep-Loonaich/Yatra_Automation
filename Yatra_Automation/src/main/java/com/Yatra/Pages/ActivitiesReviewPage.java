...
public class ActivitiesReviewPage extends LoadableComponent<ActivitiesReviewPage> {
	...
	@FindBy(css = "input[class='upper-case col-xs-12 ng-pristine ng-untouched ng-valid']")
	private WebElement txtPromoCode;
	
	@FindBy(css = "button[ng-class='promoCont.applyButtonClass']")
	private WebElement btnApplyNow;
	
	@FindBy(css = "h3[class='fs-base normal gray-dark mt-1 ng-binding']")
	private WebElement txtInvalidPromoCode;
	...
	public boolean verifyYatraUserCheckboxInLogin() throws Exception {
		return BrowserActions.isRadioOrCheckBoxSelected(driver
				.findElement(By.cssSelector("div[class='row']>label>span[class='account-msg my-checkbox']>input")));
	}
	...
	public void enterPromoCode(String PromoCode) throws Exception {
		BrowserActions.typeOnTextField(txtPromoCode, PromoCode, driver, "Enter Promo Code");
		BrowserActions.clickOnElement(btnApplyNow, driver, "Click on Apply Now");
	}
	
	/***
	 * Getting Error Message From Invalid Promo Code
	 * 
	 * @param email
	 * @throws Exception
	 */
